...
const projects = [
  ...
  },
  {
    ...
    result: "Katalog online dengan order via WhatsApp",
  ...
  },
  {
    ...
    result: "Check-in QR dengan laporan real-time",
      ...
      <div className="mx-auto max-w-6xl">
        <div className="mb-12 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div className="max-w-2xl">
            <p className="mb-4 text-sm font-bold uppercase tracking-wide text-primary">Bukti, bukan janji</p>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {projects.map((project) => (
            <article key={project.title} className="group overflow-hidden rounded-3xl border border-border bg-card shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
              <div className={`relative flex h-56 items-end overflow-hidden bg-gradient-to-br ${project.visual} p-6`}>
                <div className="absolute right-8 top-8 h-32 w-24 rotate-6 rounded-xl border border-white/40 bg-white/25 shadow-xl transition-transform duration-500 group-hover:rotate-12 group-hover:scale-105" />
                <div className="absolute right-16 top-14 h-32 w-24 -rotate-6 rounded-xl border border-white/30 bg-white/15" />
              ...
              <div className="flex flex-col gap-4 p-6">
                ...
                <div className="mt-2 flex items-center gap-2 border-t border-border pt-4 text-sm font-semibold text-primary"><CircleCheck className="size-4" /> {project.result}</div>
              </div>
            </article>
          ))}
        </div>
